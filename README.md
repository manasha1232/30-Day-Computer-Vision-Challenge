<div align="center">
# 👁️ 30-Day Computer Vision Challenge
### *30 Days • 30 Hands-On Projects • OpenCV, YOLOv11 & MediaPipe*
[![Challenge Progress](https://img.shields.io/badge/Progress-11%2F30%20Days%20(37%25)-brightgreen?style=for-the-badge&logo=target)](https://github.com/manasha1232)
[![Python](https://img.shields.io/badge/Python-3.9%2B-blue?style=for-the-badge&logo=python&logoColor=white)](https://www.python.org/)
[![OpenCV](https://img.shields.io/badge/OpenCV-5.0.0-green?style=for-the-badge&logo=opencv&logoColor=white)](https://opencv.org/)
[![YOLOv11](https://img.shields.io/badge/YOLO-v11-orange?style=for-the-badge&logo=ultralytics&logoColor=white)](https://github.com/ultralytics/ultralytics)
[![Stars](https://img.shields.io/github/stars/manasha1232/30-Day-Computer-Vision-Challenge?style=for-the-badge&color=yellow)](https://github.com/manasha1232/30-Day-Computer-Vision-Challenge/stargazers)
[![License](https://img.shields.io/badge/License-MIT-red?style=for-the-badge)](LICENSE)
---
### 📊 Challenge Progress Tracker
```text
[███████████████░░░░░░░░░░░░░░░░░░░░░░░░░░░░░] 11 / 30 Days Completed (36.7%)
⭐ If you find this repository useful for learning Computer Vision, please consider giving it a STAR! ⭐

📌 Overview
This repository is a curated 30-Day Computer Vision & Deep Learning Challenge. It takes developers from core image processing fundamentals (filtering, edge detection, color spaces) to advanced real-world AI applications including multi-object tracking, real-time driver safety monitoring, retail shelf inventory auditing, and automated volume scanning.

Each day contains:

💻 Fully functional, well-commented Python source code
🖼️ Sample input test images/videos
📊 Annotated output results, JSON logs & summary reports
📄 Dedicated project README.md
🗺️ 30-Day Project Roadmap
🚀 Phase 1: Core Fundamentals & Filtering (Days 1–5)
Day	Project Title	Technologies	Status	Link
01	Computer Vision Fundamentals	OpenCV, NumPy	COMPLETED	📁 View Repo
02	Real-Time Yellow Color Detection	OpenCV, HSV Masking	COMPLETED	📁 View Repo
03	Real-Time Face Blur	MediaPipe, OpenCV	COMPLETED	📁 View Repo
04	YOLO Object Detection & Tracking	YOLOv11, OpenCV	COMPLETED	📁 View Repo
05	Driver Drowsiness & Yawning Monitor	MediaPipe, OpenCV	COMPLETED	📁 View Repo
🛍️ Phase 2: Retail & Industrial AI Vision (Days 6–10)
Day	Project Title	Technologies	Status	Link
06	Smart Shelf Restock Alert System	YOLOv11, OpenCV, JSON	COMPLETED	📁 View Repo
07	Book Spine Scanner & Cataloger	YOLOv11, OpenCV	COMPLETED	📁 View Repo
08	Document Scanner & Perspective Warper	OpenCV, Contours	COMPLETED	📁 View Repo
09	Optical Flow Motion Vector Tracker	OpenCV, Lucas-Kanade	COMPLETED	📁 View Repo
10	Virtual Hand Gesture Controller	MediaPipe Hands, OpenCV	COMPLETED	📁 View Repo
🚗 Phase 3: Traffic, Security & Motion Systems (Days 11–20)
Day	Project Title	Technologies	Status	Link
11	Self-Driving Lane Line Detector	OpenCV, Hough Lines	COMPLETED	📁 View Repo
12	Real-Time Object Dimension Measurer	OpenCV, Contours	UPCOMING	⏳ In Progress
13	Automated License Plate Recognition (ANPR)	YOLOv11, OCR	UPCOMING	⏳ Planned
14	Vehicle Speed Estimator	YOLOv11, Centroid Tracking	UPCOMING	⏳ Planned
15	AI Fitness Rep Counter & Pose Detector	MediaPipe Pose, OpenCV	UPCOMING	⏳ Planned
(Days 16–30: Deep Learning, Segmentation & Generative Vision Modules)

🌟 Featured Project Highlights
🛒 [Day 06] Smart Shelf Restock Alert System
Repository: Smart_Shelf_Restock_Sysytem
Highlights: Real-time retail inventory monitor dividing shelves into ROIs, auditing stock counts, and triggering RESTOCK REQUIRED alert banners.
📚 [Day 07] Book Spine Scanner & Cataloger
Repository: book_spine_scanner
Highlights: Scans bookshelves, computes spine dimensions & thickness categories (Slim, Medium, Thick Volume), extracts dominant color signatures, and exports structured JSON databases.
📄 [Day 08] Document Scanner & Perspective Warper
Repository: document_scanner_perspective_warper
Highlights: Detects 4-corner document boundaries on cluttered backgrounds, applies 4-point homography perspective warping, and generates CamScanner quality adaptive B&W and color magic scans.
🌀 [Day 09] Optical Flow Motion Vector Tracker
Repository: optical_flow_motion_tracker
Highlights: Features Sparse Lucas-Kanade feature trails with forward-backward error filtering, Farneback Dense flow HSV heatmaps with vector quiver overlays, and real-time 3-panel HUD telemetry.
🖐️ [Day 10] Virtual Hand Gesture Controller
Repository: virtual_hand_gesture_controller
Highlights: Detects 21 3D skeletal hand landmarks, classifies gesture triggers (Open Palm, Fist, Peace Sign, Pointing, Pinch, Thumbs Up/Down), and renders interactive volume sliders and HUD controls.
🚗 [Day 11] Self-Driving Lane Line Detector
Repository: self_driving_lane_line_detector
Highlights: Autonomous vehicle lane tracking module using HSL color masking, Canny edge detection, Probabilistic Hough Lines, EMA curve smoothing, and Top-Down Bird's Eye View perspective warping.
⚡ Quickstart



# Clone the repository
git clone https://github.com/manasha1232/30-Day-Computer-Vision-Challenge.git
cd 30-Day-Computer-Vision-Challenge
# Install dependencies
pip install -r requirements.txt
💡 Why Star This Repository?
📖 Comprehensive Reference: 30 practical CV solutions with clean Python code.
🏢 Industry Relevant: Focuses on real-world use cases (Retail, Safety, Automation, Security).
🚀 1-Click Runnable: Includes sample test images for immediate testing.
👤 Author & Connect
GitHub: @manasha1232
Star History: Please click ⭐ at the top right if this project helps your CV journey!
